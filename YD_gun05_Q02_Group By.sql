...
-- urunleri urun adina gore gruplandir
-- VE her bir grup icin SUM(miktar_kg) degerini yazdir
...
SELECT kategori, SUM(miktar_kg*satis_fiyati)
FROM market
GROUP BY kategori;
...
-- once urunler ve satis miktarlarini listeleyen bir sorgu yazalim

SELECT urun_adi, SUM(miktar_kg)
FROM market
GROUP BY urun_adi;

-- yukardaki sorgu bize urunleri ve toplam miktarlarini iceren 
-- bir tablo getirir, 
-- simdi bu gelen tablo icinde en yuksek degere sahip kaydi bulmak icin
-- kayitlari siralayip, 1.olani isteyebiliriz

SELECT urun_adi, SUM(miktar_kg) AS toplam_satis_miktari
FROM market
GROUP BY urun_adi
...
SELECT SUM(miktar_kg) AS "Satilan toplam meyve miktari"
FROM market
